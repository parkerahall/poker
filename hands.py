...
class Hands:
	...
	def get_highest_hand(card_list):
		...
		for suit in suit_dict:
			...
			if len(suited_set) >= 5:
				high_hand = Hands.check_straight(suited_set)
				if high_hand == 14:
					return Hands("ROYAL FLUSH", ())
				elif high_hand != None:
					return Hands("STRAIGHT FLUSH", (high_hand,))
				else:
					highest_value = suited_set[0].value
					best_hand = Hands("FLUSH", (highest_value,))

		# check for normal straight
		high_straight = Hands.check_straight(sorted_list)
		if high_straight != None:
			if best_hand == None:
				best_hand = Hands("STRAIGHT", (high_straight,))
		...
		if biggest_set == 4:
			...
			return Hands("FOUR OF A KIND", (set_value, kicker))
		elif biggest_set == 3:
			set_value = max(reverse_dict[biggest_set])
			kicker = -1
			for value in reverse_dict[biggest_set]:
				if value != set_value and conv_to_int(value) > kicker:
					kicker = conv_to_int(value)
			for value in reverse_dict.get(2, []):
				if conv_to_int(value) > kicker:
					kicker = conv_to_int(value)
			kicker_value = conv_to_str(kicker)
			if kicker > -1:
				return Hands("FULL HOUSE", (set_value, kicker_value))
			else:
				if best_hand == None:
					return Hands("THREE OF A KIND", (set_value, kicker_value))
				else:
					return best_hand
		elif biggest_set == 2:
